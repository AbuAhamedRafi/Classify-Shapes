...
function Classification() {
  ...
  const predictDrawing = async (category) => {
    ...
    const categoryMappings = {
      ...
      letter: {
        ...
        model: letterModel,
        mappingFunction: (predictedClass) =>
          String.fromCharCode(letterMapping(predictedClass - 1)),
      ...
      digit: {
        ...
        model: digitModel,
        mappingFunction: (predictedClass) =>
          String.fromCharCode(digitMapping(predictedClass)),
      ...
      merged: {
        ...
        model: mergedModel,
        mappingFunction: (predictedClass) =>
          String.fromCharCode(mergedMapping(predictedClass)),
    ...
    setTimeout(async () => {
      ...
      const { canvas, model, mappingFunction, resultStateUpdater, rotate } =
        categoryMapping;
  ...
  return (
    <div className="bg-gray-100 bg-stripes">
      {/* Main content */}
      <div
        className={`flex flex-wrap md:w-[500px] md:mx-auto h-screen justify-center ${
          mergeScreen ? "lg:w-[500px]" : "lg:w-[1400px]"
        }`}
      >
        <h1 className="absolute top-4 text-gray-600 tracking-widest flex gap-1">
          ...
          <button
            onClick={() => setMergeScreen(!mergeScreen)}
            className="bg-gray-300 px-2 rounded-md shadow-sm py-0.5"
          >
            {mergeScreen ? "Separated Models" : "Switch to Merged Model"}
          </button>
        </h1>
